...
<div class="mt-4">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between bg-gray-50 border border-gray-200 rounded-lg px-4 py-3 gap-3">
        <div>
            <p class="text-sm font-medium text-gray-800">Anasayfa Carousel'inde Göster</p>
            <p class="text-xs text-gray-500 mt-0.5">Bu hizmet ana sayfadaki hizmetler carousel'inde listelensin.</p>
        </div>
        @php($showOnHome = old('show_on_home', $service->show_on_home ?? true))
        <label class="relative inline-flex items-center cursor-pointer select-none">
            <input type="checkbox" name="show_on_home" value="1" class="sr-only peer" {{ $showOnHome ? 'checked' : '' }}>
    ...
    @error('show_on_home')<div class="text-red-600 text-sm mt-2">{{ $message }}</div>@enderror
</div>
