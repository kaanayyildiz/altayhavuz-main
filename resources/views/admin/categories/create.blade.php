...
@section('title', 'Yeni Kategori - Admin')

@section('content')
    <h1 class="text-2xl font-bold mb-4">Yeni Kategori</h1>

    <form action="{{ route('admin.categories.store') }}" method="POST" class="bg-white border rounded-xl p-6 space-y-5 max-w-2xl">
        @csrf
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Ad (TR)</label>
            <input type="text" name="name_tr" value="{{ old('name_tr') }}" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
        ...
        <div>
            ...
            <input type="text" name="name_en" value="{{ old('name_en') }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
        ...
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.categories.index') }}" class="px-4 py-2 rounded-lg border">İptal</a>
