...
@section('title', 'Menü - Admin')

@section('content')
    <div class="flex items-center justify-between mb-4">
        <h1 class="text-2xl font-bold">Menü</h1>
        <a href="{{ route('admin.menus.create') }}" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">Yeni Öğe</a>
    </div>

    <div class="bg-white border rounded-xl overflow-hidden">
        <div class="px-4 py-3 bg-gray-50 border-b flex items-center justify-between">
            <div class="text-sm text-gray-600">Sürükle-bırak ile sıralamayı değiştirebilirsiniz.</div>
            <button id="saveMenuOrder" class="hidden bg-blue-600 hover:bg-blue-700 text-white text-sm px-3 py-1.5 rounded">Sıralamayı Kaydet</button>
        </div>
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody id="sortableMenuBody" class="divide-y divide-gray-200">
                @forelse($menus as $menu)
                    <tr data-id="{{ $menu->id }}" class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3">{{ $menu->title_tr }}</td>
                        <td class="px-4 py-3">{{ $menu->url }}</td>
                        <td class="px-4 py-3">
                            <span class="px-2 py-1 text-xs rounded {{ $menu->status === 'active' ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-700' }}">{{ $menu->status }}</span>
                        </td>
                        <td class="px-4 py-3 text-right space-x-2">
                            <a href="{{ route('admin.menus.edit', $menu) }}" class="text-blue-600 hover:underline">Düzenle</a>
                            <form action="{{ route('admin.menus.destroy', $menu) }}" method="POST" class="inline" onsubmit="return confirm('Silmek istediğinize emin misiniz?')">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-4 py-6 text-center text-gray-500">Henüz menü öğesi yok.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <script>
        (function(){
            const el = document.getElementById('sortableMenuBody');
            if(!el) return;
            const saveBtn = document.getElementById('saveMenuOrder');
            new Sortable(el, {
                handle: '.cursor-move',
                animation: 150,
...
@section('title', 'Menüler - Admin')

@section('content')
    <div class="flex items-center justify-between mb-4">
        <h1 class="text-2xl font-bold">Menüler</h1>
        <a href="{{ route('admin.menus.create') }}" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">Yeni Menü</a>
    </div>
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse($menus as $m)
                    <tr>
                        ...
                        <td class="px-4 py-3 text-right space-x-2">
                            <a href="{{ route('admin.menus.edit', $m) }}" class="text-blue-600 hover:underline">Düzenle</a>
                            <form action="{{ route('admin.menus.destroy', $m) }}" method="POST" class="inline" onsubmit="return confirm('Silmek istediğinize emin misiniz?')">
                                @csrf
                                @method('DELETE')
                                <button class="text-red-600 hover:underline">Sil</button>
                            </form>
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-4 py-6 text-center text-gray-500">Henüz menü yok.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
